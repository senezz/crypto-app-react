...
export default function AppContent() {
 ...
 const totalProfit = portfolio
              .map((asset) => asset.amount * cryptoPriceMap[asset.id] - asset.price * asset.amount)
              .reduce((acc, v) => (acc += v), 0)
              .toFixed(2)
 ...
 return (
    <Layout.Content style={contentStyle}>
      <Typography.Title level={3} style={{ textAlign: 'left', color: '#fff' }}>
        ...
        <Flex gap='20px' justify="flex-start" align='center' style={{ width: '100%' }}>
          <Statistic
            value={Math.abs(totalProfit)}
            precision={2}
            styles={{ content: { color: totalProfit > 0 ? '#60be0e' : '#e63948' }}}
